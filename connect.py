...
from utils.userassets import get_user_assets
...
class TTYNav:
    def __init__(self, username=username_login):
        self.username = username
        self.userassets = list(get_user_assets(self.user))
    ...
    def print_nav(self):
        ...
        1) 输入 \033[32mIP或者编号\033[0m 登录设备
    ...
    def print_user_assets(self):
        print('%-5s %-16s\t%-16s\t%s' % ('Index', 'Hostname', 'IP', 'User'))
        for i, asset in enumerate(self.userassets, 1):
            print('%-5s %-16s\t%-16s\t%s' % (i, asset.hostname, asset.ip, asset.username))
    ...
    def dispatch(self):
        while True:
            ...
            if input_ == 'p':
                ...
                try:
                    obj = self.userassets[int(input_)-1]
                    input_ = obj.ip
                except:
                    pass
                self.connect(input_)
